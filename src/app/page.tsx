...
export default function HomePage() {
  ...
  const navigateFile = useCallback(
    (direction: 'next' | 'prev') => {
      ...
      const currentIdx = items.findIndex((i) => i.path === selectedItem.path);
      if (currentIdx === -1) return;

      const fileItems = items.filter((i) => i.type === 'file');
      const fileIdx = fileItems.findIndex((i) => i.path === selectedItem.path);
      if (fileIdx === -1) return;

      const newIdx = direction === 'next' ? fileIdx + 1 : fileIdx - 1;
      if (newIdx < 0 || newIdx >= fileItems.length) return;

      const newItem = fileItems[newIdx];
      setSelectedItem(newItem);
      if (viewMode === 'edit' && !isTextFile(newItem.extension)) {
        setViewMode('preview');
      }
    },
    [selectedItem, items, viewMode]
  );

  // 滑动手势 - 浏览模式右滑返回上级
  const browseSwipe = useSwipeGesture({
    onSwipeRight: handleGoBack,
    threshold: 80,
  });

  // 滑动手势 - 预览/编辑模式
  const previewSwipe = useSwipeGesture({
    onSwipeRight: handleBackToBrowse,
    onSwipeLeft: () => navigateFile('next'),
    threshold: 80,
  });
  ...
  }, [pullRefreshing, currentPath, loadDirectory]);
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* 主内容区 */}
        <main className="flex-1 flex flex-col overflow-hidden" {...(viewMode === 'browse' ? browseSwipe : previewSwipe)}>
          {/* 移动端：预览/编辑模式下显示返回按钮 */}
          {viewMode !== 'browse' ? (
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            {viewMode !== 'browse' && selectedItem && (
              <div className="flex-1 h-full overflow-hidden" {...previewSwipe}>
